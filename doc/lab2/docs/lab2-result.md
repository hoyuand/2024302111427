...
- Lab1 banner 与 `expect_banner.txt` 前缀保持一致；两次冷启动和 Lab1 资产测试通过。
...
- `bufstorm` 串口驱动对短行测试的 `BUFSTORM lines=4 bytes=8` 和后续 `sh>` 全部断言通过；四条 149 字节输入的压力测试输出 `BUFSTORM lines=4 bytes=262`，剩余片段被 Shell 作为命令拒绝，但内核持续响应且未发生死锁或 panic。
...
完整 QEMU 原始输出保存在 [`lab2-build-and-smoke.txt`](lab2-build-and-smoke.txt)、[`lab2-spin-output.txt`](lab2-spin-output.txt)、[`lab2-bufstorm-output.txt`](lab2-bufstorm-output.txt) 和 [`lab2-overflow-output.txt`](lab2-overflow-output.txt)。自动串口断言结果见 `lab2-*-driver.txt`。
