...
main(void)
{
  ...
  printf("OSLAB1 sid=%ld mod97=%x\n", (int64)COURSE_SID,
         (uint)(COURSE_SID % 97));

  /* 自检序列：覆盖 0、负数、最大/最小 int、空字符串、%x 与 %%。 */
  printf("printf-boundary zero=%d neg=%d max=%d min=%d empty=\"%s\" hex=%x percent=%%\n",
         0, -42, 2147483647, (-2147483647 - 1), "", 42);
  printf("printf-long=");
